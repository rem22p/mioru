...
func TestIntegrationListCategories(t *testing.T) {
	...
	// Eyewear (migration 018) — walk the tree and confirm at least the
	...
	walk(cats)
	if !found {
		t.Error("category tree does not contain eyewear (slug=eyewear); migration 018 may not have been applied")
	}
}
