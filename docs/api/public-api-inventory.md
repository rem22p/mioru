...
| Method+Path | Auth | CSRF | RL | Success | Errors | Integration test |
|---|---|---|---|---|---|---|
| GET /api/products | — | — | — | 200 `{products,total,page,per_page}` (the key is `products`, not `items` — the storefront reads it as such). Filter arrays capped: 20 values each (`brand`, `color`, `size`), 100 for `category_id` (picking a root category sends every descendant id), ≤60 runes per brand / ≤40 per color / ≤32 per size; `page` ≤ 100000 — oversized → 400 VALIDATION_FAILED. **search** is clamped store-side to the first 200 runes (not bytes — a byte cut would split a multi-byte rune and make Postgres reject the query). **sort** ∈ `{popular, newest, created_at, price, -price, name, brand}` — empty or unknown value = `created_at DESC` (newest); `popular` uses `popularity_rank` (`popularity_rank_preorder` when `status=preorder`); `brand` sorts by the derived display name | — | `TestIntegrationListProducts` (+ store-level paginate/filter) |
