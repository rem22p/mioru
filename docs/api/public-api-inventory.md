...
| POST /api/store/orders | ✓ | ✓ | — | 201 order | 400 VALIDATION_FAILED (incl. missing/invalid `phone`, stale product_id → `PRODUCT_NOT_FOUND`, `cod + bus` for cart orders, `moldovaPost` for PMR cities), 409 IDEMPOTENCY_REPLAY, 409 INSUFFICIENT_STOCK | `TestIntegrationCreateOrder*` (happy/stock/replay/conflict/oversell/missing-key), `TestIntegrationCreateOrderRequiresPhone`, `TestIntegrationCreateOrderPersistsPhoneAndSyncsToProfile`, `TestIntegrationCreateOrderDoesNotResyncIdenticalPhone`, `TestIntegrationCreateOrderRejectsUnknownProduct`, `TestIntegrationCreateOrderRejectsCodWithBus`, `TestIntegrationCreateOrderAllowsCodWithBusForIndividual`, `TestIntegrationCreateOrderRejectsMoldovaPostInPNR` |
