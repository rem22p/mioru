...
- **Ship wired vertical slices, not orphaned halves** — a backend route/store method/migration merges only together with the caller that exercises it (and a frontend store/action merges only with the API it talks to). Code that nothing references is *unfinished work, not "done"*: a `PUT /…/cart` endpoint that no client ever calls, or a Zustand store that persists to `localStorage` while the matching server endpoint sits unused, is a non-feature that still carries full review/security/migration cost. The PR title and description state what the diff actually *wires end-to-end*, never the eventual intent. If a half is deliberately deferred, say so explicitly and keep the unwired half out of `main` until its other end lands.
...
- **Schema = versioned migrations** via **tern** (`github.com/jackc/tern/v2/migrate`). `internal/store/migrations/NNN_*.sql`, `//go:embed`-ed; `runMigrations()` (called from `PostgresStore.migrate()`) applies up to head on startup, tracking the version in `public.schema_version`. `001_baseline.sql` is the original schema as `CREATE TABLE IF NOT EXISTS` (existing DBs adopt it as a no-op); `002_seed_categories.sql` seeds the category tree. New schema/seed changes ship as new numbered files — never edits to applied ones. **A migration number is free only relative to `main` *at merge time*, not at branch time** — two open branches that both grabbed the next `NNN` (e.g. `006_orders.sql` and `006_customer_cart_favorites.sql`) collide on whichever merges second, and tern refuses to apply duplicate/non-sequential versions, breaking startup on that deploy. The branch merged second renumbers its migration on rebase; verify your `NNN` is still unused against `main` before merging, never assume the number you picked at branch time survived. Migrations **never hardcode environment- or person-specific values** (usernames, hosts, keys): a literal like `WHERE username = 'rem22p'` is a silent no-op on any other deployment and bakes one operator into shared schema. Role/first-admin assignment is driven by `seedAdmin` / `BOOTSTRAP_ADMIN_*` or a stable criterion (e.g. the oldest admin), idempotently — one source of truth, not a migration and a seeder both claiming it.
...
All authenticated calls are **cookie-based**: the SPA sends `credentials: "include"` and, on `POST`/`PUT`/`PATCH`/`DELETE`, copies the matching CSRF cookie into `X-CSRF-Token`. There is no `Authorization: Bearer …` path and no token in the JSON body — login/register return only the user/customer profile. `Access-Control-Allow-Headers` advertises only `Content-Type, X-CSRF-Token, Idempotency-Key` (`Idempotency-Key` for finance-critical mutations; `Authorization` is intentionally absent).

- **Error envelope** (all 4xx and 5xx): `{ "error": "<human>", "code": "<MACHINE_CODE>" }`. The SPA branches on `code`, not on the text. Reserved codes: `AUTH_REQUIRED`, `AUTH_INVALID`, `CSRF_INVALID`, `FORBIDDEN`, `RATE_LIMITED`, `VALIDATION_FAILED`, `NOT_FOUND`, `CONFLICT`, `IDEMPOTENCY_REPLAY`, `INSUFFICIENT_STOCK`, `TELEGRAM_REQUIRED`, `PAYLOAD_TOO_LARGE`, `INTERNAL`. 5xx carries only `{"error":"Internal error","code":"INTERNAL"}` — never `err.Error()`. **Every** error response — including those emitted from middleware (authz/CSRF gates) — goes through the JSON envelope helper carrying a machine `code`; never `http.Error`, which sends `text/plain` with no `code` and breaks the SPA's `code`-based branching.
...
- CORS is an explicit allowlist (`CORS_ORIGINS`); credentialed responses reflect only allowlisted origins (`Vary: Origin`). `Allow-Headers` exposes only `Content-Type, X-CSRF-Token, Idempotency-Key` — `Authorization` is not advertised because Bearer auth is intentionally absent.
...
- **API surface changes ship with tests + docs** — any change to the public API surface (a new/renamed/removed route, an altered request or response shape, a status code, an error `code`, or an auth/CSRF/role/rate-limit gate) updates its handler-level integration test (happy path **plus** the gates it touches) and the `docs/api/` inventory/contract reference in the **same** PR. A route whose behaviour changed but whose test did not is treated as untested. Reference harness: `backend/api/internal/handler/integration_*_test.go` (real PostgreSQL + the real middleware chain).
- **UI changes ship with tests** — any change to a user-facing flow, page, or interactive component (a new/changed route, form, cart/checkout/auth step, or a control that triggers a state change or API call) ships with coverage in the **same** PR: Vitest + Testing Library for component/store logic, and **Playwright** E2E for the user flows. Playwright is the E2E tool for both SPAs — store specs live in `apps/store/e2e/*.spec.ts`; admin gets its own `apps/admin/e2e/` (auth-gated flows: login, product CRUD, uploads, super-admin user mgmt). Priority-#1 user journeys (login → catalog → cart → order) are functional E2E (assert behaviour + the real `POST /api/store/orders`), not just visual-regression snapshots. A UI flow that changed but whose test did not is treated as untested.
- **Stable selectors via `data-testid`** — E2E and component tests target `data-testid`, never i18n copy or Tailwind classes (both churn and produce flaky specs). Every flow-critical interactive element carries a stable `data-testid` (auth email/password/submit, size pick, add-to-cart, cart row/total/checkout, checkout city/step/confirm/success, order rows). When you touch such an element keep its testid intact; when you add a new flow step add a testid for it. A testid that a spec references is a contract — renaming or removing it without updating the spec in the same PR breaks the suite.
