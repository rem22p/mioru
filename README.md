...
|---|---|---|
| `APP_ENV` | Окружение. При `production`/`prod` пустой или короткий `SECRET_KEY` — фатальная ошибка старта (не подменяется случайным) | `development` |
| `SECRET_KEY` | Ключ подписи JWT, ≥32 символов. **В production обязателен.** Вне production при отсутствии генерируется случайно (токены сбрасываются при рестарте) | — (в dev генерируется) |
